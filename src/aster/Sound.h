...
class SoundEngine {
public:
    SoundEngine(void){m_Volume=1.0; m_bPause=false;}
    virtual ~SoundEngine(void){Close();}
    virtual bool Open(void)=0;
    virtual void Close(void){}
    virtual void SetVolume(float in_Vol){m_Volume=in_Vol;}//ustawia glosnosc dzwiekow w skali 0-1. 
    virtual float GetVolume(void){return m_Volume;}//gloscnosc dzwiekow w skali 0-1. 0 oznacza wylaczona
    virtual void Mute()=0;
...
protected:
    bool m_bPause;
    float m_Volume;
    virtual bool InitSound(void)=0;
    virtual void FreeSound(void)=0;
};
