...
#define FNTSIZELARGE 25
	...
	{
		m_bDispFPS = false;
		m_pFontSmall = new ast::GameFont("vector battle", FNTSIZESMALL);
		m_pFontLarge = new ast::GameFont("vector battle", FNTSIZELARGE);

		m_pGameOverText = new CTextControl(m_pFontLarge, ast::Rectanglei(0, geWorld.scrHeight, 0, geWorld.scrWidth));
		m_pGameOverText->SetAlignement(CTextControl::taCenter);
